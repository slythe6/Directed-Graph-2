...
{ 
 ...
 public:  // available to the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: Returns top element of the stack
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: returns true if stack is empty, false if stack is not empty
  bool isEmpty();

  //PURPOSE: returns true if stack is full, false if not full
  bool isFull();

  //PURPOSE: displays all current elements of stack
  void displayAll();

  //PURPOSE: empties stack by cycling through and popping each element
  void clearIt();
